...
procedure TForm6.CreationObjet(X, Y: integer; Click: TNotifyEvent; red: boolean = false; green: boolean = false);
var
	objet : TImage;
Begin
	objet := TImage.Create(Form6);
	with objet do
	Begin
		Parent := Form6; // L'attache à la fenêtre de Jeu
		if red then
...
procedure TForm6.CheckPos (chessChecker: boolean; var point, base: TPoint; var posss: TList; masque: integer);
var
	tmp: integer;
	tmpb: byte;
	tmpp: TPoint;
...
function TForm6.GetPoss (typePion, x, y: integer; chessChecker: boolean = false): TList;
var
	pos, tmp: TPoint;
	posss: TList;
	i: Integer;
...
procedure TForm6.DestroyHandler (var Msg: TMessage);
begin
	while nbBlue <> 0 do
	begin
		Blues[nbBlue].Free;
...
procedure TForm6.ExecutionHandler (var Msg: TMessage);
var
	i: integer;
	tmp: TImage;
	tmpp: TPoint;
...
procedure TForm6.ChessHandler (var Msg: TMessage);
var
	i, j: integer;
...
procedure TForm6.ImageClick(Sender: TObject);
var
	i : Integer;
	pos: TPoint;
	sen: TImage;
