...
var
	...
	nbBlue, nbChessWhite, nbChessBlack: integer;
...
procedure TForm5.CreationObjet(X, Y: integer; Click: TNotifyEvent; red: boolean = false; green: boolean = false);
var
	objet : TImage;
Begin
	objet := TImage.Create(Form5);
	with objet do
	Begin
		Parent := Form5; // L'attache à la fenêtre de Jeu
		if red then
...
procedure TForm5.CheckPos (chessChecker: boolean; var point, base: TPoint; var posss: TList; masque: integer);
var
	tmp: integer;
	tmpb: byte;
	tmpp: TPoint;
...
function TForm5.GetPoss (typePion, x, y: integer; chessChecker: boolean = false): TList;
var
	pos, tmp: TPoint;
	posss: TList;
	i: Integer;
...
procedure TForm5.DestroyHandler (var Msg: TMessage);
begin
	while nbBlue <> 0 do
	begin
		Blues[nbBlue].Free;
...
procedure TForm5.ExecutionHandler (var Msg: TMessage);
var
	i: integer;
	tmp: TImage;
begin
...
procedure TForm5.ChessHandler (var Msg: TMessage);
var
	i, j: integer;
...
begin
	...
	if (WhiteChess) and (Msg.wParam = 0) then
	begin
	...
	if (BlackChess) and (Msg.wParam = 0) then
	begin
...
procedure TForm5.SelectionnableClick(Sender: TObject);
var
	sen: TImage;
...
begin
	...
	begin
		...
		begin
			...
			SendMessage (self.Handle, UM_CHESS, 1, 0);
	...
	begin
		...
		begin
			...
			PostMessage (self.Handle, UM_CHESS, 1, 0);
		end
		else
		begin
...
procedure TForm5.ImageClick(Sender: TObject);
var
	i : Integer;
	pos: TPoint;
	sen: TImage;
